...
describe('DeepSeekProvider tool-selection capability', function () {
    it('declares what it can force, so callers can ask instead of catching', function () {
        expect(is_subclass_of(DeepSeekProvider::class, NamedToolSelectableInterface::class))->toBeTrue();
        expect(is_subclass_of(DeepSeekProvider::class, ToolSelectableInterface::class))->toBeTrue();
    });
});
